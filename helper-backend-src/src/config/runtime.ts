...
export type RuntimeNetworkConfig = {
  ...
  lokiUrl: string;
  lokiPublicUrl?: string;
  infisicalUrl: string;
  infisicalPublicUrl?: string;
...
const fingerprintConfig = (config: RuntimeProjectConfig) =>
...
const runRuntimeMutation = async <T>(action: () => Promise<T>) => {
  const previous = runtimeMutation;
  let release!: () => void;
  runtimeMutation = new Promise<void>((resolve) => {
    release = () => resolve();
  });
...
const normalizeNetwork = (payload: BootstrapPayload): RuntimeNetworkConfig => {
  ...
  return {
    ...
    grafanaPublicUrl: String(source.grafanaPublicUrl ?? "").trim() || undefined,
    lokiUrl: String(source.lokiUrl ?? env.LOKI_URL).trim(),
    lokiPublicUrl: String(source.lokiPublicUrl ?? "").trim() || undefined,
    infisicalUrl: String(source.infisicalUrl ?? env.INFISICAL_URL).trim(),
    infisicalPublicUrl: String(source.infisicalPublicUrl ?? "").trim() || undefined,
...
export const configureRuntime = async (
  ...
  runRuntimeMutation(async () => {
    ...
    const current = runtimeState.config ? cloneConfig(runtimeState.config) : null;

    if (current) {
      const currentFingerprint = fingerprintConfig(current);
      ...
      if (!options?.replace) {
        throwStatus(
          "Runtime is already configured. Re-submit the same payload or use replace=true to rotate backend state.",
          409
        );
      }
    }
...
export const getServiceCandidates = () => {
  const network = getRuntimeNetwork();

  return {
    grafana: [network.grafanaUrl, network.grafanaPublicUrl].filter(Boolean) as string[],
    loki: [network.lokiUrl, network.lokiPublicUrl].filter(Boolean) as string[],
    infisical: [network.infisicalUrl, network.infisicalPublicUrl].filter(Boolean) as string[],
  } as const;
};
...
export const getRuntimeNetwork = () => {
  ...
  return config?.network ?? {
    ...
    lokiUrl: env.LOKI_URL,
    lokiPublicUrl: undefined,
    infisicalUrl: env.INFISICAL_URL,
    infisicalPublicUrl: undefined,
    internalNetworkCidr: env.INTERNAL_NETWORK_CIDR,
    wgNetworkCidr: env.WG_NETWORK_CIDR,
  };
};
