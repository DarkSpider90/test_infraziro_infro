...
import { ensureConnected, reconnect, refreshWireGuardStatus } from "../services/wireguard.js";
...
export const registerHealthRoutes = async (app: FastifyInstance) => {
  ...
  app.post("/api/v1/wireguard/ensure", async () => {
    return ensureConnected(app.log);
  });

  app.post("/api/v1/wireguard/reconnect", async () => {
    return reconnect(app.log);
  });
